...
import { updateAgentProfile } from 'src/apis/agents'
...
const TabAccount = () => {
  ...
  const [imgSrc, setImgSrc] = useState(user.image_url??'/images/avatars/15.png')
  ...
  const handleUploadFiles = async() => {
    if(file){
      ...
      try {
        ...
        console.error('Error uploading avatr: ', error);
        toast.error('Error uploading avatr');
  ...
  const onSubmitForm = async(values) => {
    ...
    if(fileUploaded){
      formData.image_url=fileUploaded[0].image_url
      formData.image_key=fileUploaded[0].image_key
     }else{
      formData.image_url=null
      formData.image_key=null
     }
